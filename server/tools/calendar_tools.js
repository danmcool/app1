...
CalendarTools.computeQuery = function (startTime, endTime) {
    ...
    if (startTime.getDay() == endTime.getDay() && (endTime.getTime() - startTime.getTime() < Constants.OneDay)) {
        ...
        for (var i = 1; i < (endDay - startDay); i++) {
            ...
            currentDate.setTime(startTime.getTime() + i * Constants.OneDay);
            var currentDateKey = computeDateKey(currentDate);
...
var removeBusyInterval = function (intervals, user, object) {
    ...
    if (!removed) {
        console.log('Error removing interval for user: ' + user + ' object id: ' + object);
    }
    return interval;
}
    ...
    if (added) {
        ...
        for (var j = 0; j < intervals.length; j++) {
            for (var k = 0; k < intervals.length; k++) {
                if (intervals[j].end == intervals[k].start) {
                    intervals[j].end = intervals[k].end;
                    intervals.splice(k, 1);
                    removed = true;
                    break;
                }
                if (intervals[j].start == intervals[k].end) {
                    intervals[j].start = intervals[k].start;
                    intervals.splice(k, 1);
                    removed = true;
                    break;
                }
...
var removeFreeInterval = function (intervals, start, end) {
    ...
    if (!removed) {
        console.log('Error removing interval: ' + start + ' ' + end);
    }
}
...
CalendarTools.removeEvent = function (appointments, appointment_properties, startDate, endDate, user, object) {
    ...
    if (startDate.getDay() == endDate.getDay() && (endDate.getTime() - startDate.getTime() < Constants.OneDay)) {
        ...
        if (appointments[startDateKey].busy.length == 0) {
            delete appointments[startDateKey];
        } else {
            addFreeInterval(appointments[startDateKey].free, interval.start, interval.end);
        }
        ...
        if (appointments[endDateKey].busy.length == 0) {
            delete appointments[endDateKey];
        } else {
            addFreeInterval(appointments[endDateKey].free, interval.start, interval.end);
        }
        for (var i = 1; i < (endDay - startDay); i++) {
            var currentDate = new Date();
            currentDate.setTime(startDate.getTime() + i * Constants.OneDay);
            var dateKey = computeDateKey(currentDate);
            interval = removeBusyInterval(appointments[dateKey].busy, user, object);
            if (appointments[dateKey].busy.length == 0) {
                delete appointments[dateKey];
            } else {
                addFreeInterval(appointments[dateKey].free, interval.start, interval.end);
            }
        }
    }
    return true;
...
CalendarTools.addEvent = function (appointments, appointment_properties, startDate, endDate, user, object) {
    ...
    if (startDate.getDay() == endDate.getDay() && (endDate.getTime() - startDate.getTime() < Constants.OneDay)) {
        ...
        for (var i = 1; i < (endDay - startDay); i++) {
            ...
            addBusyInterval(appointments[dateKey].busy, startTimeProperties, endTimeProperties, user, object);
            removeFreeInterval(appointments[dateKey].free, startTimeProperties, endTimeProperties);
        }
        return true;
    }
}
