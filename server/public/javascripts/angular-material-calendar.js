...
angular.module("materialCalendar").service("MaterialCalendarData", [function () {
    function CalendarData() {
        ...
        this.setDayAvailable = function (date, available) {
            this.available[this.getDayKey(date)] = available || this.available[this.getDayKey(date)] || "";
        };
    }
    ...
    return {
        ...
        link: function ($scope, $element, $attrs) {
            ...
            $scope.isDisabled = function (date, startDateOfMonth, noOfDays) {
                ...
                if (!$scope.dataService.available[$scope.dayKey(date)]) {
                    return true;
                }
            ...
            var getDayAvailable = function (date) {
                // Initialize the data in the data array.
                if ($scope.noCache) {
                    $scope.dataService.setDayAvailable(date, "");
                } else {
                    $scope.dataService.setDayAvailable(date, ($scope.dataService.available[getDayKey(date)] || ""));
                }

                var cb = ($scope.dayAvailable || angular.noop)();
                var result = (cb || angular.noop)(date);

                // Check for async function. This should support $http.get() and also regular $q.defer() functions.
                if (angular.isObject(result) && "function" === typeof result.success) {
                    result.success(function (available) {
                        $scope.dataService.setDayAvailable(date, available);
                    });
                } else if (angular.isObject(result) && "function" === typeof result.then) {
                    result.then(function (available) {
                        $scope.dataService.setDayAvailable(date, available);
                    });
                } else {
                    $scope.dataService.setDayAvailable(date, result);
                }
            };
            ...
            var setAvailable = function () {
                angular.forEach($scope.calendar.weeks, function (week) {
                    angular.forEach(week, getDayAvailable);
                });
            };
